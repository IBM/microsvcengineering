...
# --------------------------------------------------------------------------------- #
# This file has been changed to add java agent jmx_prometheus_javaagent-0.12.0.jar
# to the Kafka startup command
# --------------------------------------------------------------------------------- #
...
CLASSPATH=${CLASSPATH#:}
JAVA_AGENT=-javaagent:${base_dir}/libs/jmx_prometheus_javaagent-0.12.0.jar=${KAFKA_HOST}:${KAFKA_PORT}:${base_dir}/config/config.yml
...
if [ "x$DAEMON_MODE" = "xtrue" ]; then
  nohup $JAVA $JAVA_AGENT $KAFKA_HEAP_OPTS $KAFKA_JVM_PERFORMANCE_OPTS $KAFKA_GC_LOG_OPTS $KAFKA_JMX_OPTS $KAFKA_LOG4J_OPTS -cp $CLASSPATH $KAFKA_OPTS "$@" > "$CONSOLE_OUTPUT_FILE" 2>&1 < /dev/null &
else
  exec $JAVA $JAVA_AGENT $KAFKA_HEAP_OPTS $KAFKA_JVM_PERFORMANCE_OPTS $KAFKA_GC_LOG_OPTS $KAFKA_JMX_OPTS $KAFKA_LOG4J_OPTS -cp $CLASSPATH $KAFKA_OPTS "$@"
fi
